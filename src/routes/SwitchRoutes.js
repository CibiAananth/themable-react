...
const SwitchRoutes = ({ routes }) => (
  <Switch>
    {routes.map((prop, key) => {
      const { redirect, subRoutes, path, to, component: Component } = prop;
      if (redirect) return <Redirect from={path} to={to} key={key} />;
      if (subRoutes !== undefined && subRoutes.length !== 0)
        return switchSubRoutes(subRoutes, key);
      return (
        <Route
          path={path}
...
const switchSubRoutes = routes =>
  routes.map((prop, itemKey) => {
    const { subRoutes, path, component: Component } = prop;
    if (subRoutes !== undefined && subRoutes.length !== 0)
      return switchSubRoutes(subRoutes, itemKey);
    return (
      <Route
        path={path}
        render={rp => <Component {...prop} {...rp} />}
